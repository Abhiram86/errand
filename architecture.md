...
## Background task scheduler — `schedule_task` + `TaskSchedulerService` + AlarmManager

One-off and recurring tasks execute headlessly with no UI involved:

- **Tool** (`lib/tools/schedule_task_tool.dart`) — `create`/`edit`/`delete`/`get`/`list`/`logs` over the `SchedulerTasks` table, with optional per-task `model`/`provider_id` overrides stored in `payloadJson`. Lenient arg coercion (doubles/numeric strings); strict failures on invalid `status`/`schedule_type`.
- **Report collection** (`lib/tools/headless/report_tool.dart`) — headless-only `save_report(content, name?, type?)`. The model supplies content; the handler owns everything exact: sandboxed `task-<id>-<startedAt>[-<name>].<ext>` filename (basename-sanitized, `md|html|txt` allowlist, 500KB cap, last-call-wins) and records the path in a per-run `HeadlessReportCollector`. `AgentRunner.runHeadless` prefers the collected path and falls back to writing the final answer text, so a report file always exists.
- **Execution** (`TaskSchedulerService.executeTask`) — allowlist guard (`scheduled`/`failed` only) + atomic claim to `running`, 10-minute timeout with `CancelToken`, recurring reschedule with `retriesPerTurn` cap, `failures` reset on success, stuck-`running` recovery sweep, 250-char notification bodies, and pruning to the newest 10 reports per task.
- **Native wake-up** (`TaskAlarmManager`/`TaskAlarmReceiver`/`TaskBootReceiver`/`TaskExecutionService` + `TaskAlarmRestorer`) — exact alarms with inexact fallback, boot/update re-registration by reading the scheduler table directly (no Flutter needed), serialized queue with engine-readiness handshake, and `NotificationHelper` channels.
- **UI** (`ManageTasksScreen`) — Upcoming/Unread/All tabs with filters, countdown ticker, per-task model override sheet, file previews, unread banner, and a session-scoped storage footer.
...
- `AppSettings { key PK, value }` — encrypted secrets + plain preferences (v3).
- `SchedulerTasks { id PK, title, type (one_off/recurring), status, payloadJson (prompt + optional model/providerId overrides), startsAt, nextRunAt?, repeatAfter?, timezone, notify, lastRunAt?, totalRuns, failures, retriesPerTurn, createdAt, updatedAt }` (v7) — `SchedulerTaskLogs { id PK, schedulerTaskId FK CASCADE, scheduledFor, startedAt?, finishedAt?, status (running/success/failed/timeout/cancelled), noAttempts, errorMessage?, outputFilePath?, summary?, notificationSent, notificationSeen, createdAt, updatedAt }` with indexes on `(status, next_run_at)` and `(scheduler_task_id, scheduled_for DESC)`.
...
`schemaVersion = 7`, `NativeDatabase` (or `drift_flutter` on device), `inMemory()` for tests. v1→v2 migration dedupes message rows, then creates the two indexes above; v2→v3 adds the app-settings table; v3→v4 adds `attachedUrisJson`; v4→v5 adds the `memories` table; v5→v7 adds the scheduler tables plus their indexes.
...
11. `lib/main.dart` + `lib/screens/chat_screen.dart` + `lib/widgets/`
