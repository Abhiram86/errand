...
> **Last updated:** 2026-09-01 | **Status:** P0-P3 shipped (v0.2.1), P4a #1-2 done | **Schema:** Drift v4
...
ChatScreen (lib/main.dart)  owns Conversation, _messages, _pendingAttachments,
  + Drift watches + WorkingDirectory + LlmClient + model picker + paging
    │
AgentLoop (lib/agent/agent_loop.dart:35)  18 turns, _toLlmHistory, trimLlmMessages
...
Persistence: ErrandDatabase (lib/services/database.dart) Drift v4
...
ModelCatalog: GET {baseUrl}/models → architecture.input_modalities → ModelOption.supportsInput
```
...
| `intent_service.dart` | `IntentService` (launch/canResolve/hasWriteSettings/FGS indicator) |
| `model_catalog.dart` | `ModelCatalogService` (cache+inFlight dedup, supportsInput) |
...
|---|-----------|---------|---------|---------------|
| 1 | **1** | `ModelOption` | `lib/models/model_option.dart` | `inputModalities` default `["text"]`, provider from id prefix |
| 2 | 6 | `LlmClient`+`CancelToken` | `lib/llm/llm_client.dart:21,94` | builder closure per retry, `_ownsClient` abort, `_sseDataEvents` utf8 reassembly |
| 3 | 7 | `AgentLoop`+`ContextBudget`+`ToolRegistry` | `lib/agent/agent_loop.dart:35` `context_budget.dart` `tool_registry.dart:15` `tool.dart` | 18 turns, `truncateHistory`@boundary + `trimLlmMessages` mid-loop, media as user role |
| 4 | 3 | `read` (text+structured+media) | `lib/tools/file_tools.dart` | 20 MiB pre-check, `_resolveReadableFile` cache escape, `contentParts` synthetic, 32K clamp |
...
| 7 | 5 | `intent` tool | `lib/tools/intent_tool.dart` | 17 curated → `android_action` hatch → honest fail; `isReopenable` open-style only |
...
| 13 | — | Types | `lib/types/*.dart` | `Conversation.attachedFileUris` global, `ToolCallResult.contentParts` |
| 14 | — | Theme | `lib/theme/app_colors.dart` | `kDarkBg` etc. dark M3 |
| 15 | 9 | Native Kotlin | `android/.../MainActivity.kt` `ErrandAccessibilityService.kt` `AgentForegroundService.kt` | `FLAG_ACTIVITY_NEW_TASK`, `resolveActivity` only if pinned, `isRestricted` AppOps |
...
```
1  model_catalog.dart + model_option.dart          ← smallest, isolated      [x] 2026-08-29
2  tavily_client.dart + web_tools                  ← web layer               [x] 2026-09-01
3  workspace.dart + file_tools.dart + workspace_tool + attached_files_tool  [ ]
4  internal/document_reading/ (4 files)            ← structured readers      [ ]
5  intent_service.dart + intent_tool.dart + MainActivity.kt                  [ ]
6  llm_client.dart (+ CancelToken/retry)           ← resilience core         [ ]
7  agent/context_budget.dart + agent_loop.dart     ← truncation+loop cap     [ ]
8  services/database.dart (schema v4)              ← includes attachedUrisJson [ ]
9  a11y_service.dart + ErrandAccessibilityService.kt + screen/act tools      [ ]
10 main.dart + widgets last                        ← depends on all above    [ ]
```
...
- **Context budget:** `kContextSoftLimit 200K`, keep ≤110K, `kMaxSingleResult 32K`, tool batch atomic, last `UserMessage` mandatory. Boundary `truncateHistory` + mid-loop `trimLlmMessages`.
- **Intent safety:** curated→generic→honest fail. URL `https://` prefix only for `_looksLikeWebHost`, block `javascript:/file:`. `resolveActivity` only when package pinned.
...
## 7. Open Items (carry from next_plan.md:436)

- Stream-stall watchdog (~20 LOC inactivity transformer, headers timeout only)
- Malformed SSE JSON toast cosmetic
- `maxTurns` 18 → ~40 param + mid-run compaction (for long a11y flows, keep last 6 turns)
- `write`/`edit_file` + local retrieval (backlog post-P4b)
- `_OpenXmlPackage.load` streaming zip hardening
